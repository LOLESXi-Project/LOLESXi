(async function () {
  ...
  try {
    elements = await window.loadLOLESXIThreatGraphData();

    if (loadingBox) {
      loadingBox.innerHTML = `
        <p class="panel-title">Status</p>
        <p class="empty-state">Loaded ${elements.length} graph elements from /api/lolesxi.json.</p>
      `;
    }
  } catch (error) {
    console.error("[LOLESXi Threat Graph] Failed to load graph data", error);

    if (loadingBox) {
      loadingBox.innerHTML = `
        <p class="panel-title">Status</p>
        <p class="empty-state">Could not load /api/lolesxi.json. Check browser console for details.</p>
      `;
    }

    document.getElementById("graph-details").innerHTML = `
      <div class="detail-card">
        <span class="node-type">Error</span>
        <h2>Graph data failed to load</h2>
  ...
  /* Procedure map is now the default view. */
  let currentMode = "procedures";
  ...
  function getActivityForActor(actorNode) {
    const taggedBinaries = actorNode
      .outgoers("edge[relationship='tagged binary']")
      .targets();

    const directProcedures = actorNode
      .outgoers("edge[relationship='uses procedure']")
      .targets();

    let activity = actorNode
      .union(actorNode.connectedEdges())
      .union(taggedBinaries)
      .union(directProcedures);

    if (currentMode === "procedures") {
      const proceduresFromBinaries = taggedBinaries
        .outgoers("edge[relationship='contains procedure']")
        .targets();

      const procedureEdges = proceduresFromBinaries
        .union(directProcedures)
        .connectedEdges();
  ...
  function buildThreatActorFilter() {
    ...
    if (actors.length === 0) {
      actorFilterContainer.innerHTML = `<p class="empty-state">No threat group tags were found in the API.</p>`;
      return;
    }

    actorFilterContainer.innerHTML = actors.map(actor => {
      const binaryCount = actor
        .outgoers("edge[relationship='tagged binary']")
        .targets()
        .length;

      const procCount = actor
        .outgoers("edge[relationship='uses procedure']")
        .targets()
        .length;
  ...
  function applyThreatActorFilter() {
    ...
    selectedThreatActors.forEach(actorId => {
      ...
      if (actorNode && actorNode.nonempty()) {
        visibleElements = visibleElements.union(getActivityForActor(actorNode));
      }
    });
  ...
  function applyTypeFilters(refit = true) {
    ...
    cy.nodes(":visible").forEach(node => {
      if (!checkedTypes.includes(node.data("type"))) {
        node.hide();
      }
    });
  ...
  function setMode(mode) {
    ...
    document.getElementById("view-overview")?.classList.toggle("active", mode === "overview");
    document.getElementById("view-procedure-map")?.classList.toggle("active", mode === "procedures");
  ...
  function updateDetailsDefault() {
    ...
    details.innerHTML = `
      <div class="detail-card">
        ...
        <h2>Procedure map default</h2>
        <p>
          The graph is loaded directly into Procedure map view.
  ...
  /* Procedure map is the default loaded view. */
  setMode("procedures");
